...
# CustomProperties UserDefinedPin (...) line — captures everything inside outer parens.
_USER_DEFINED_PIN_RE = re.compile(
    r"CustomProperties\s+UserDefinedPin\s+\((.+)\)\s*$"
)
...
def _parse_pin(content: str) -> BlueprintPin:
    ...
    for key, value in tokens:
        if key == "PinId":
        ...
        elif key == "PinType.PinSubCategoryMemberReference":
            # value is "(content)" — extract inner content
            inner = value
            if inner.startswith("(") and inner.endswith(")"):
                inner = inner[1:-1]
            pin_sub_category_member_ref = inner
        elif key == "PinType.PinValueType":
            # value is "(content)" — extract inner content
            inner = value
            if inner.startswith("(") and inner.endswith(")"):
                inner = inner[1:-1]
            pin_value_type = inner
            ...
            default_value = value
        elif key == "DefaultObject":
            default_object = value
        elif key == "DefaultTextValue":
            ...
            not_connectable = _parse_bool(value)
        elif key == "bDefaultValueIsIgnored":
    ...
    return BlueprintPin(
        ...
        pin_sub_category_member_ref=pin_sub_category_member_ref,
        pin_value_type=pin_value_type,
...
def _parse_node_body(
    ...
    for line in body.split("\n"):
        ...
        # Check for CustomProperties UserDefinedPin line
        udp_match = _USER_DEFINED_PIN_RE.search(stripped)
        if udp_match:
            raw_content = udp_match.group(1)
            udp_dict: dict[str, str] = {"raw": raw_content}
            for tok_key, tok_val in _tokenize_pin_content(raw_content):
                udp_dict[tok_key] = tok_val
            user_defined_pins.append(udp_dict)
            continue
    ...
    kwargs: dict = dict(
        ...
        user_defined_pins=user_defined_pins,
    )
    if node_guid:
