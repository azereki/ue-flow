...
- **Connection Validator:** `api/connection-validator.ts` — `canConnect()` validates pin compatibility (direction, category with float↔real compat, implicit type conversions via `canImplicitlyConvert()`, wildcard, no self-connections, no duplicates, single exec input rule, exec output auto-replacement via `replaces` field, enum subCategoryObject matching)
...
- **Node Palette:** `components/NodePalette.tsx` — searchable palette (Tab key or right-click canvas) with 2,700+ functions from signature DB + special entries (events, flow control, variables, comments, reroute, 8 cast nodes, Break/Make for 12 UE structs)
...
- **Dynamic Pins:** `nodes/DynamicPinButton.tsx` — "+" button for adding pins on Sequence, MakeArray, Select, operator, and SwitchInteger nodes. `utils/dynamic-pins.ts` manages config and pin generation
