...
- **Context Menu:** `components/ContextMenu.tsx` — right-click menu on nodes (Duplicate, Delete, Add Note/Edit Note), edges (Delete Connection), and connected pin handles (Delete Connection(s)). Right-click targets are isolated — nodes/edges/handles never fall through to the node palette. Pan suppression uses `isPanningRef` (set by `onMoveStart`, cleared only on next right-button mousedown — no rAF timing race)
- **Node Palette:** `components/NodePalette.tsx` — searchable palette (Tab key or right-click empty canvas only, guarded by `.react-flow__pane` check) with 2,700+ functions from signature DB + special entries (events, component events, enhanced input, flow control incl. ForLoop/WhileLoop/Timeline, delegates, spawning, variables, utility incl. Self/PrintString/IsValid, comments, reroute, 8 cast nodes, Break/Make for 12 UE structs)
