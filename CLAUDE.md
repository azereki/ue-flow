...
- **AI Provider:** OpenRouter is the sole AI provider. `AIProviderContext.tsx` manages API key, model selection, and `chatCompletion()`. Users configure via `AISettings.tsx` popover. `openrouter.ts` has `MODEL_OPTIONS` (curated free/budget/standard/premium tiers) and API client. Keys stored in session or localStorage based on "remember" preference
